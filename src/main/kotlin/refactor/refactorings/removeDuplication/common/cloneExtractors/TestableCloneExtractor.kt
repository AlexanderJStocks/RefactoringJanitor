...
class TestableCloneExtractor : CloneExtractor() {
    ...
    override fun applyRefactoringAndTest(
        cu: CompilationUnit, cloneGroup: List<MethodDeclaration>, projectRoot: Path
    ...
    private fun copyRefactoringChanges(
        cu: CompilationUnit, cloneGroup: List<MethodDeclaration>, newMethod: MethodDeclaration
    ) {
        val originalMethods = cu.findAll(MethodDeclaration::class.java)
        cloneGroup.forEach { clone ->
            println(originalMethods.first { it.nameAsString == clone.nameAsString && it.parameters == clone.parameters }
                .replace(clone))
        }
        ...
        val cuTargetClass = targetClassOrInterface?.findAll(ClassOrInterfaceDeclaration::class.java)
            ?.first { it.nameAsString == target.nameAsString }
        val cuTargetEnum =
            targetEnum?.findAll(EnumDeclaration::class.java)?.first { it.nameAsString == target.nameAsString }
